...
RedisResult* CRedisHelper::SelectDbCommand(RedisConn* poRedisConn,UINT32 dwIdx)
{
	redisReply* reply = NULL;
	if (poRedisConn->m_poRedisConn)
	{
		reply = (redisReply*)redisCommand(poRedisConn->m_poRedisConn, "select %u", dwIdx);
	}
	if (!CheckReply(reply))
	{
...
// scan 0 match *actordata:1:* 
RedisResult* CRedisHelper::ZscanCommand(RedisConn* poRedisConn,const char * szKey,UINT32 dwCursor)
{
	redisReply* reply = NULL;
	if (poRedisConn->m_poRedisConn)
	{
		reply = (redisReply*)redisCommand(poRedisConn->m_poRedisConn, "scan %u match %s", dwCursor, szKey);
	}
	if (!CheckReply(reply))
	{
		if(poRedisConn->Reconnect())
...
RedisResult* CRedisHelper::GetRedisString(RedisConn* poRedisConn, const char* table, const char* key)
{
	...
	if (poRedisConn->m_poRedisConn)
	{
		reply = (redisReply*)redisCommand(poRedisConn->m_poRedisConn, "get %s:%s:%s", table, key, DBConfig::Instance()->GetKeySuffix().c_str());
	}
	if (!CheckReply(reply))
	{
		if(poRedisConn->Reconnect())
...
RedisResult* CRedisHelper::MGetRedisString(RedisConn* poRedisConn, const char* table, std::vector<std::string>& key)
{
	...
	{
		ss << " "<< table << ":" << key[i].c_str() <<":" << DBConfig::Instance()->GetKeySuffix().c_str();
	}
	redisReply* reply = NULL;
	if (poRedisConn->m_poRedisConn)
	{
...
RedisResult* CRedisHelper::SetRedisTimeLiness(RedisConn* poRedisConn, const char* table, const char* key, UINT32 time)
{
	...
	if (poRedisConn->m_poRedisConn)
	{
		reply = (redisReply*)redisCommand(poRedisConn->m_poRedisConn, "expireat %s:%s:%s %u", table, key,DBConfig::Instance()->GetKeySuffix().c_str(),  time);
	}
	if (!CheckReply(reply))
	{
		if(poRedisConn->Reconnect())
...
RedisResult* CRedisHelper::SetRedisString(RedisConn* poRedisConn, const char* table, const char* key, const std::string& value)
{
	...
	if (poRedisConn->m_poRedisConn)
	{
		reply = (redisReply*)redisCommand(poRedisConn->m_poRedisConn, "set %s:%s:%s %b", table, key, DBConfig::Instance()->GetKeySuffix().c_str(), value.c_str(), value.length());
	}
	if (!CheckReply(reply))
	{
		if(poRedisConn->Reconnect())
